...
def simple(g):
	...
	# YOUR CODE HERE
	edge1 = g.edges
	edge2 = edge1.map(lambda x: [x[1],x[0]]).toDF(("src", "dst")).distinct()
	edge3 = edge1.unionAll(edge2)
	v = g.vertices
	g = GraphFrame(v, edge3)
	return g
...
def readFile(filename, large, sqlContext=sqlContext):
	lines = sc.textFile(filename,1000)
	...
	# YOUR CODE HERE
	pair = lines.map(lambda line:line.split(delim))
	edge = sqlContext.createDataFrame(pair,['src','dst'])
	...
	# graphframe vertices
	# YOUR CODE HERE
	v = edge.selectExpr('src as id').unionAll(edge.selectExpr('dst as id')).distinct()
