...
def closeness(g):
	...
	# YOUR CODE HERE
	verticeList = g.vertices.select('id').collect()
	...
	for vertex in verticeList:
		pathList.append(str(vertex.id))
	df = g.shortestPaths(landmarks = pathList).select(functions.explode('distances')).groupby('key').agg(functions.sum('value').alias('s')).selectExpr('key as id','1/s as closeness')
	return df
...
df1 = closeness(g).sort('closeness',ascending=False)
df1.show()
df1.toPandas().to_csv("centrality_out.csv")
